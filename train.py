...
def train_main(args):
    ...
    for epoch in range(1, args.max_epoch + 1):
        ...
        log_data = [int(epoch), float("{:.3f}".format(train_acc)), float("{:.4f}".format(train_loss)), 
                    float("{:.3f}".format(val_acc)), float("{:.4f}".format(val_loss)), int(isBetterModel)]

        if len(log_list) != 0:
            log_list = np.vstack([log_list, log_data])
        else: 
            log_list = log_data

        lr_scheduler.step()

    return model, log_list


if __name__ == '__main__':
    args = setup_run(arg_mode='train')

    model, log_list = train_main(args)

    df = pd.DataFrame(log_list, columns = ['Epoch', 'Train_avg.acc', 'Train_avg.loss', 'Val_avg.acc', 'Val_avg.loss', 'isBetterModel'])
    df.set_index('Epoch', inplace=True)
    df.to_csv(os.path.join(args.save_path, 'Train_log_data.csv'))
